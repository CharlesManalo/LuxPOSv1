...
export function LoginPage() {
      ...
      if (stored) {
        ...
        if (user.role === 'admin') {
          ...
          if (tenant) {
            ...
            const userBranch = branches.find((b: { id: string }) => b.id === user.branch_id);
  ...
  const roleConfig = {
    cashier: { icon: Coffee, label: 'Cashier Login', color: 'bg-accent-orange', desc: 'Access the POS system' },
    owner: { icon: Store, label: 'Owner Login', color: 'bg-success-green', desc: 'Manage your business' },
    admin: { icon: Shield, label: 'Admin Access', color: 'bg-[#2c2c2c]', desc: 'System administration' },
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: '#f5f5f5' }}>
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-[#2c2c2c] font-heading">LuxPOS</h1>
        ...
        <div className="bg-white rounded-3xl shadow-lg p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-lg font-semibold text-[#2c2c2c] font-heading">{config.label}</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-sm font-medium text-[#2c2c2c] mb-1.5 block">Email</label>
              <Input
                type="email"
                value={email}
            ...
            <div>
              <label className="text-sm font-medium text-[#2c2c2c] mb-1.5 block">Password</label>
          ...
          <div className="mt-6 pt-4 border-t border-[#e0e0e0]">
            ...
            <div className="grid grid-cols-1 gap-1.5 text-xs text-muted-foreground">
              ...
              >
                <span className="font-medium text-[#ff9e2c]">Cashier:</span> maria@silogan.ph
              </button>
              <button
                type="button"
                ...
                className="text-left px-3 py-1.5 rounded-lg hover:bg-[#f5f5f5] transition-colors"
              >
                <span className="font-medium text-[#00c29f]">Owner:</span> juan@silogan.ph
              </button>
              <button
                type="button"
                ...
                className="text-left px-3 py-1.5 rounded-lg hover:bg-[#f5f5f5] transition-colors"
              >
                <span className="font-medium text-[#2c2c2c]">Admin:</span> admin@luxpos.app
              </button>
            </div>
          </div>
        </div>

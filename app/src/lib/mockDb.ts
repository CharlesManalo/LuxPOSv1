import type {
  ...
  Ingredient, ProductIngredient, Order, OrderItem, InventoryLog, Notification,
  PaymentMethod, UserRole,
} from '@/types';
import { getStoredData, setStoredData, generateId } from './mockData';

// Auth
export async function signIn(email: string, _password: string): Promise<{ user: AppUser | null; error: string | null }> {
  await delay(400);
  ...
  if (!user) {
    ...
    if (email === 'juan@silogan.ph') {
      ...
      return { user: owner || null, error: null };
    }
    if (email === 'maria@silogan.ph') {
      const cashier = data.users.find((u) => u.role === 'cashier' && u.branch_id === 'branch-1');
      return { user: cashier || null, error: null };
    }
    if (email === 'pedro@silogan.ph') {
      const cashier = data.users.find((u) => u.role === 'cashier' && u.branch_id === 'branch-2');
      return { user: cashier || null, error: null };
    }
    return { user: null, error: 'User not found. Try admin@luxpos.app, juan@silogan.ph, maria@silogan.ph, or pedro@silogan.ph' };
  }
...
export async function createBranch(branch: Omit<Branch, 'id' | 'created_at'>): Promise<Branch> {
  await delay(300);
  const data = getStoredData();
  const newBranch: Branch = { ...branch, id: generateId('branch'), created_at: new Date().toISOString() };
...
export async function createCategory(category: Omit<Category, 'id' | 'created_at'>): Promise<Category> {
  await delay(300);
  const data = getStoredData();
  const newCat: Category = { ...category, id: generateId('cat'), created_at: new Date().toISOString() };
...
export async function getProducts(tenantId: string, categoryId?: string): Promise<Product[]> {
  await delay(200);
  let products = getStoredData().products.filter((p) => p.tenant_id === tenantId);
  if (categoryId) {
...
export async function createProduct(product: Omit<Product, 'id' | 'created_at'>): Promise<Product> {
  await delay(300);
  const data = getStoredData();
  const newProduct: Product = { ...product, id: generateId('prod'), created_at: new Date().toISOString() };
...
export async function deleteProduct(id: string): Promise<void> {
  ...
  data.productVariants = data.productVariants.filter((v) => v.product_id !== id);
  data.productIngredients = data.productIngredients.filter((pi) => pi.product_id !== id);
  setStoredData({ products: data.products, productVariants: data.productVariants, productIngredients: data.productIngredients });
}

// Product Variants
export async function getProductVariants(productId: string): Promise<ProductVariant[]> {
  await delay(150);
  return getStoredData().productVariants.filter((v) => v.product_id === productId);
}

export async function createVariant(variant: Omit<ProductVariant, 'id' | 'created_at'>): Promise<ProductVariant> {
  await delay(200);
  const data = getStoredData();
  const newVariant: ProductVariant = { ...variant, id: generateId('var'), created_at: new Date().toISOString() };
...
// Ingredients
export async function getIngredients(tenantId: string, branchId?: string): Promise<Ingredient[]> {
  await delay(200);
  let ingredients = getStoredData().ingredients.filter((i) => i.tenant_id === tenantId);
  if (branchId) {
...
export async function createIngredient(ingredient: Omit<Ingredient, 'id' | 'created_at'>): Promise<Ingredient> {
  await delay(300);
  const data = getStoredData();
  const newIng: Ingredient = { ...ingredient, id: generateId('ing'), created_at: new Date().toISOString() };
...
export async function deleteIngredient(id: string): Promise<void> {
  ...
  data.productIngredients = data.productIngredients.filter((pi) => pi.ingredient_id !== id);
  setStoredData({ ingredients: data.ingredients, productIngredients: data.productIngredients });
}

// Product Ingredients
export async function getProductIngredients(productId: string): Promise<ProductIngredient[]> {
  await delay(150);
  return getStoredData().productIngredients.filter((pi) => pi.product_id === productId);
}

export async function setProductIngredients(productId: string, ingredients: { ingredient_id: string; qty_required: number }[]): Promise<void> {
  await delay(300);
  const data = getStoredData();
  data.productIngredients = data.productIngredients.filter((pi) => pi.product_id !== productId);
  for (const ing of ingredients) {
    data.productIngredients.push({ id: generateId('pi'), product_id: productId, ingredient_id: ing.ingredient_id, qty_required: ing.qty_required });
  }
  setStoredData({ productIngredients: data.productIngredients });
}

// Orders
export async function getOrders(tenantId: string, branchId?: string, limit?: number): Promise<Order[]> {
  await delay(200);
...
export async function getOrder(id: string): Promise<Order | null> {
  ...
  return { ...order, branch_name: branch?.name, cashier_name: cashier?.full_name };
}

export async function createOrder(order: Omit<Order, 'id' | 'created_at'>, items: Omit<OrderItem, 'id'>[]): Promise<Order> {
  await delay(400);
  const data = getStoredData();
  const newOrder: Order = { ...order, id: generateId('order'), created_at: new Date().toISOString() };
  data.orders.push(newOrder);

  // Create order items
  const newItems: OrderItem[] = items.map((item) => ({ ...item, id: generateId('oi') }));
  data.orderItems.push(...newItems);

  // Deduct inventory
  for (const item of items) {
    const prodIngredients = data.productIngredients.filter((pi) => pi.product_id === item.product_id);
    for (const pi of prodIngredients) {
      const ingredient = data.ingredients.find((i) => i.id === pi.ingredient_id && i.branch_id === order.branch_id);
      if (ingredient) {
  ...
  setStoredData({ orders: data.orders, orderItems: data.orderItems, ingredients: data.ingredients, inventoryLogs: data.inventoryLogs });
  return newOrder;
}

export async function voidOrder(orderId: string, approvedBy: string): Promise<Order> {
  await delay(300);
  const data = getStoredData();
  ...
  setStoredData({ orders: data.orders, ingredients: data.ingredients, inventoryLogs: data.inventoryLogs });
...
// Inventory
export async function getInventoryLogs(tenantId: string, branchId?: string): Promise<InventoryLog[]> {
  await delay(200);
  let logs = getStoredData().inventoryLogs.filter((l) => l.tenant_id === tenantId);
  if (branchId) {
...
export async function restockIngredient(ingredientId: string, qty: number, triggeredBy: string): Promise<Ingredient> {
  await delay(300);
  const data = getStoredData();
  ...
  setStoredData({ ingredients: data.ingredients, inventoryLogs: data.inventoryLogs });
  return data.ingredients[idx];
}

// Notifications
export async function getNotifications(tenantId: string, branchId?: string): Promise<Notification[]> {
  await delay(200);
  let notifs = getStoredData().notifications.filter((n) => n.tenant_id === tenantId);
  if (branchId) {
    notifs = notifs.filter((n) => n.branch_id === branchId || n.branch_id === null);
  }
...
export async function createNotification(notification: Omit<Notification, 'id' | 'created_at'>): Promise<Notification> {
  await delay(300);
  const data = getStoredData();
  const newNotif: Notification = { ...notification, id: generateId('notif'), created_at: new Date().toISOString() };
  data.notifications.push(newNotif);
  setStoredData({ notifications: data.notifications });
  return newNotif;
}

export async function updateNotificationStatus(id: string, status: 'approved' | 'rejected', resolverId: string): Promise<Notification> {
  await delay(300);
  const data = getStoredData();
...
export async function getDashboardStats(tenantId: string, branchId?: string, days: number = 30): Promise<DashboardStats> {
  await delay(300);
  const data = getStoredData();
  const cutoff = new Date();
  cutoff.setDate(cutoff.getDate() - days);

  let orders = data.orders.filter((o) => o.tenant_id === tenantId && o.status === 'completed' && new Date(o.created_at) >= cutoff);
  if (branchId) orders = orders.filter((o) => o.branch_id === branchId);

  const totalRevenue = orders.reduce((s, o) => s + o.total, 0);
  const totalOrders = orders.length;
  const totalProducts = data.products.filter((p) => p.tenant_id === tenantId).length;

  let ingredients = data.ingredients.filter((i) => i.tenant_id === tenantId);
  if (branchId) ingredients = ingredients.filter((i) => i.branch_id === branchId);
  const lowStockCount = ingredients.filter((i) => i.stock_qty <= i.low_stock_threshold).length;

  const methods: PaymentMethod[] = ['cash', 'gcash', 'maya'];
  const paymentBreakdown = methods.map((m) => ({
    method: m,
    amount: orders.filter((o) => o.payment_method === m).reduce((s, o) => s + o.total, 0),
  ...
  for (let i = 13; i >= 0; i--) {
    ...
    dailyRevenue.push({ date: dateStr, amount: dayOrders.reduce((s, o) => s + o.total, 0) });
  }
  ...
  const branchBreakdown = branchData.map((b) => {
    ...
    return { branch_id: b.id, branch_name: b.name, revenue: bOrders.reduce((s, o) => s + o.total, 0), orders: bOrders.length };
  });

  return { totalRevenue, totalOrders, totalProducts, lowStockCount, paymentBreakdown, dailyRevenue, topProducts, branchBreakdown };
}

// Users
export async function getUsers(tenantId: string, role?: UserRole): Promise<AppUser[]> {
  await delay(200);
...
export async function createUser(user: Omit<AppUser, 'id' | 'created_at'>): Promise<AppUser> {
  await delay(300);
  const data = getStoredData();
  const newUser: AppUser = { ...user, id: generateId('user'), created_at: new Date().toISOString() };
